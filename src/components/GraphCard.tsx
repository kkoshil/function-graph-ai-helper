...
export const GraphCard: React.FC<GraphCardProps> = ({ functionExpression, data }) => {
  ...
  const handleSaveAsPng = async () => {
    ...
    setIsSaving(true);
    try {
        const canvas = await html2canvas(graphRef.current, { backgroundColor: '#ffffff', useCORS: true, logging: false, scale: 2 });
    ...
    <div ref={graphRef} className="bg-white rounded-xl shadow-lg p-4 sm:p-6 h-[400px] md:h-[500px] lg:h-full flex flex-col animate-fade-in">
      ...
      <div className="flex-grow h-full">
        <ResponsiveContainer width="100%" height="100%">
            ...
            <XAxis dataKey="x" type="number" domain={['dataMin', 'dataMax']} tick={{ fill: '#64748b', fontSize: 12 }} label={{ value: 'x', position: 'insideBottomRight', offset: -10, fill: '#334155' }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: '#64748b', fontSize: 12 }} domain={['auto', 'auto']} allowDataOverflow={true} axisLine={false} tickLine={false} />
            <ReferenceLine y={0} stroke="#94a3b8" strokeWidth={1.5} />
            <ReferenceLine x={0} stroke="#94a3b8" strokeWidth={1.5} />
            <Tooltip contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(4px)', border: '1px solid #e2e8f0', borderRadius: '0.5rem', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1)'}} labelStyle={{ color: '#1e293b', fontWeight: 'bold' }} formatter={(value: number) => [`${Number(value).toFixed(3)}`, 'y']} labelFormatter={(label: number) => `x = ${Number(label).toFixed(3)}`} />
            <Legend verticalAlign="top" align="right" wrapperStyle={{fontSize: "14px", top: "-10px"}} formatter={() => `y = ${functionExpression}`} />
